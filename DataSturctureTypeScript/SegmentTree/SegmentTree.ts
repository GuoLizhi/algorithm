...
class SegmentTree<E> {
   ...
   */
  buildSegmentTree(treeIndex: number, left: number, right: number): void {
    if (left === right) {
  ...
  private _query(treeIndex: number, left: number, right: number, queryL: number, queryR: number): E {
    ...
    if (queryL >= mid + 1) {
      ...
      let leftResult = this._query(treeIndex, left, mid, queryL, mid);
      let rightResult = this._query(treeIndex, mid + 1, right, mid + 1, queryR);
